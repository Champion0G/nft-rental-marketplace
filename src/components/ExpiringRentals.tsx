...
const ExpiringRentals = () => {
  const { expiringRentals, isLoading, error, checkExpirations } = useRentalExpiry(3600); // 1 hour threshold
  ...
  if (error) {
    return (
      <div className="card">
        <h3 className="heading">Expiring Rentals</h3>
  ...
  return (
    <div className="card">
      <div className="flex justify-between items-center mb-4">
        <h3 className="heading mb-0">Expiring Rentals</h3>
        <Button
          label="Refresh"
          onClick={checkExpirations}
          variant="secondary"
      ...
      {expiringRentals.length > 0 ? (
        ...
        <p className="text-lightText/60 text-center py-4">
          No rentals expiring in the next hour
        </p>
      )}
    </div>
  );
